...
func validateConfig(cfg *config.Config, endpoint string) error {
	...
	if endpointURL.Scheme != "https" {
		return stderrors.New("azure authenticated endpoint must use HTTPS")
	}

	if endpointURL.User != nil || endpointURL.RawQuery != "" || endpointURL.Fragment != "" {
		return stderrors.New("azure endpoint must not contain credentials, query parameters, or a fragment")
	}

	if cfg.Headers.Get("Api-Key") != "" || cfg.Headers.Get("Authorization") != "" {
		return stderrors.New("azure authentication headers must be configured through the provider")
	}
