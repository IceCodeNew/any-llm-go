...
func TestCompletionMapsCurrentThinkingRequest(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			paramsBefore, err := json.Marshal(params)
			require.NoError(t, err)
			_, err = provider.Completion(t.Context(), params)
			require.NoError(t, err)
			paramsAfter, err := json.Marshal(params)
			require.NoError(t, err)
			require.JSONEq(t, string(paramsBefore), string(paramsAfter))
...
func TestCompletionStreamMapsCurrentThinkingRequest(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{effort: providers.ReasoningEffortMinimal, want: "low"},
		{effort: providers.ReasoningEffortMax, want: "max"},
	} {
		t.Run(string(test.effort), func(t *testing.T) {
			...
			require.Equal(t, test.want, body["reasoning_effort"])
			require.Equal(t, "account_42", body["user_id"])
			require.NotContains(t, body, "user")
		})
	}
