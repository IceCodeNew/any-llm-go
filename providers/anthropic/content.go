...
func convertAssistantMessage(msg providers.Message) (*anthropic.MessageParam, error) {
	if content, ok, err := convertResponseMetadata(msg.Extra); err != nil {
		...
		if err := validateResponseMetadata(msg); err != nil {
			return nil, err
		}
...
func validateResponseMetadata(msg providers.Message) error {
	raw := msg.Extra[providerName]["response_blocks"]
	encoded, err := json.Marshal(raw)
	if err != nil {
		return fmt.Errorf("serializing Anthropic response_blocks metadata: %w", err)
	}
	var blocks []anthropic.ContentBlockUnion
	if decodeErr := json.Unmarshal(encoded, &blocks); decodeErr != nil {
		return fmt.Errorf("decoding Anthropic response_blocks metadata: %w", decodeErr)
	}
	content, reasoning, _, toolCalls, _, _, err := convertResponseContent(blocks)
	if err != nil {
		return err
	}
	if msg.ContentString() != content || !reflect.DeepEqual(msg.Reasoning, reasoning) ||
		...
		return errors.NewInvalidRequestError(
			providerName,
			fmt.Errorf("anthropic response_blocks metadata conflicts with normalized message fields"),
		)
	}
	return nil
}
